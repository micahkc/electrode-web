...
Sim plants publish `MocapFrame` FlatBuffers on the private `electrode/sim/rumoca/mocap_frame` topic only. The Ground Station selected-mocap bridge chooses either private sim mocap or the public real mocap pose and republishes exactly one compact pose stream on `synapse/mocap/selected/rigid_body/cub1/pose`. The autopilot link forwards that selected pose to cubs2, whose `csyn_decode_mocap_frame` accepts both the compact pose and the full FlatBuffer.
